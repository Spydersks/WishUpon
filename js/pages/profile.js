...
const ProfilePage = (() => {
    ...
    const render = () => {
        ...
        app.innerHTML = `
            <div class="profile-container container mx-auto px-4 py-8 md:py-12 max-w-2xl">
        ...
        return `
            ...
            <div class="space-y-4 pt-4 mt-4 border-t">
                <div class="checkbox-group">
                    <input type="checkbox" id="is-admin-checkbox" class="checkbox">
                    <label for="is-admin-checkbox" class="label font-medium" style="margin-bottom: 0;">Are you an admin?</label>
                </div>
                <div id="admin-fields" class="hidden space-y-4 admin-fields-container">
                    <div class="space-y-2">
                        <label for="admin-id" class="label">Admin ID</label>
                        <input type="text" id="admin-id" class="input" placeholder="Enter Admin ID">
                    </div>
                    <div class="space-y-2">
            ...
            <div class="checkbox-group">
                ...
                <label for="terms-checkbox" class="text-sm font-normal">
                    I agree to the <button id="terms-link" class="btn link">Terms and Conditions</button>
                </label>
            </div>`;
    };
    ...
    const getSetupButtonsHTML = () => `
        <div class="space-y-2">
            <button id="save-profile-btn" class="btn primary w-full btn-lg" disabled>
                ${UIComponents.getIcon('Save', { class: 'mr-2' })}Save and Enter App
            </button>
    ...
    const getDisplayButtonsAndQrHTML = (p) => `
        ...
        <div class="space-y-4">
             ...
             <button id="logout-btn" class="btn outline w-full">Log Out</button>
        </div>`;
    ...
    const bindEvents = () => {
        ...
        if (isSetup) {
            document.getElementById('is-admin-checkbox')?.addEventListener('change', (e) => {
                isAdminChecked = e.target.checked;
                document.getElementById('admin-fields').classList.toggle('hidden', !e.target.checked);
            });
    ...
    const handleLogout = () => {
        Api.saveUserRole(null);
        Api.saveProfile(null);
    ...
    const handleSaveProfileSetup = async () => {
        ...
        if (isAdminChecked) {
            ...
            if (adminId !== '9622248455' || adminPassword !== 'Activa12$@@') {
                return UICore.showToast({ title: "Invalid Admin Credentials", description: "The admin ID or password is not correct.", variant: "destructive" });
            }
        }
        ...
        try {
            ...
            if(saveBtn) UICore.setButtonLoading(saveBtn, false, `${UIComponents.getIcon('Save', {class: 'mr-2'})}Save and Enter App`);
        }
    };
