...
const UIHomeComponents = (() => {
    ...
    const getUpcomingBirthdayCardHTML = (person) => {
        ...
        if(daysUntil === 1) {
            ...
            countdownClass = 'text-accent font-bold';
        } else {
        ...
        return `<a href="#people" class="block birthday-card-link">
            <div class="card p-4 flex items-center justify-between transition-all">
                ...
                <div class="text-right"><div class="flex items-center gap-2 ${countdownClass} font-bold">${UIComponents.getIcon('Cake',{size:16})}<span>${countdownText}</span></div></div>
            </div>
        </a>`;
    };
